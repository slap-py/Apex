...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ snappedStops, routes: data.routes.map((route, index) => {
      const speedLimits = route.legs?.flatMap((leg) => leg.annotation?.maxspeed || []).flatMap((limit) => limit.speed ? [`${Math.round(limit.speed)} ${limit.unit || "km/h"}`] : []) || [];
      return { id: `route-${index + 1}`, coordinates: route.geometry.coordinates, distanceMeters: Math.round(route.distance), durationSeconds: Math.round(route.duration), curves: analyzeCurves(route.geometry.coordinates), speedLimits: speedLimits.filter((limit, limitIndex) => limitIndex === 0 || speedLimits[limitIndex - 1] !== limit).slice(0, 12) };
    }) });
  } catch (error) {
